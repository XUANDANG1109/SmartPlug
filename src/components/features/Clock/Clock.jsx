...
function Clock() {
  ...
  const checkSchedules = () => {
    ...
    schedules.forEach((schedule) => {
      const turnOnDateTime = new Date(
        `${schedule.date} ${schedule.turnOnTime}`
      );
      const turnOffDateTime = new Date(
        `${schedule.date} ${schedule.turnOffTime}`
      );
  ...
  const addSchedule = async () => {
    if (selectedDate && turnOnTime && turnOffTime) {
      ...
      const isoTurnOnTime = convertToISOFormat(selectedDate, turnOnTime);
      const isoTurnOffTime = convertToISOFormat(selectedDate, turnOffTime);

      const response = await axios.post(
        "https://smart-plug-api-server.onrender.com/bookings/",
        {
          start_time: isoTurnOnTime,
          end_time: isoTurnOffTime,
        }
      );
  ...
  return (
    <div className="alarm-clock bg-white dark:bg-gray-800 p-6 rounded-lg">
      ...
      <p className="text-gray-600 dark:text-gray-300 mb-6">
        Choose the exact times to turn your device on and off, ensuring energy
      ...
      <div className="device-status text-gray-700 dark:text-gray-300">
        Device Status:{" "}
        <span className={`status-${deviceStatus.toLowerCase()}`}>
          {deviceStatus}
        </span>
      </div>
            ...
            <div className="schedule-info">
              ...
              <div className="schedule-times flex gap-4 mt-2">
                <span className="text-green-500">
                  ON: {formatTime(schedule.turnOnTime)}
                </span>
                <span className="text-red-500">
                  OFF: {formatTime(schedule.turnOffTime)}
                </span>
              </div>
            </div>
            <button
